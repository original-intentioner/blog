...
<script type="text/javascript">
	...
	function deleteLink(){
		...
		if(selectedRows.length == 0){
			$.messager.alert("系统提示","请至少选择要删除的数据");
			return;
		}
		var strIds = [];
		...
		$.messager.confirm("系统提示","您确定要删除这"+selectedRows.length+"条数据吗？",function(r){
			if(r){
				$.post("${pageContext.request.contextPath}/admin/link/delete.do",{ids:ids},function(result){
					if(result.exist){
						$.messager.alert("系统提示","有类别下存在博客，不能删除");
						return;
	...
	<div id="dlg" class="easyui-dialog" style="width:500px;height:200px;padding:10px 20px"
		...
		<form action="" id="fm" method="post">
			<table cellspacing="8px">
				...
				<tr>
					<td>博客类别排序</td>
					<td><input type="text" id="orderNo" name="orderNo" class="easyui-validatebox" required="true" style="width:80px">（类别根据排序序号从小到大排序）</td>
				</tr>
			</table>
		</form>
	</div>
